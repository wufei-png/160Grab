> **重要提示（会话与保活）**  
> 实际使用中，挂号相关会话大约在十几二十分钟量级就可能失效（具体时间随站点策略变化）。当前代码里对成员页等的会话刷新**效果不稳定**，有时仍拿不到排班轮询所需的 `_user_key` / `access_hash`。参考项目 `91160-cli` 依赖账号密码与 OCR 的**自动登录**，并可定时刷新会话；本仓库默认是**手动登录接管**，不具备同款全自动续约能力。  
> 实测连接正式安装的 Chromium（有计划扩展 `chrome` 等 branded browser）时，仅靠浏览器侧定时刷新有时可以较长时间维持可用会话；后续会先尝试在这一路径上优化保活逻辑。若仍不可靠，再考虑**侵入最小**地在页面内用 JavaScript 完成必要操作。
