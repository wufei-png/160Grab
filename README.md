> **重要提示（会话与保活）**  
> 当前版本已修复“运行十几二十分钟后排班轮询因 `_user_key` / `access_hash` 丢失而中断”的主要问题；在手动登录接管模型下，脚本现在可以长期运行，并在运行时尽量复用上一次成功的排班会话 key，减少频繁刷新医生页。  
> 参考项目 `91160-cli` 依赖账号密码与 OCR 的**自动登录**做会话续约；本仓库默认仍是**手动登录接管**，不会引入同款自动登录链路。若后续真实站点策略变化导致现有保活再次不稳定，再考虑设计一条**页面内 JavaScript 直挂号**路径，尽量减少对外部轮询 key 的依赖。
